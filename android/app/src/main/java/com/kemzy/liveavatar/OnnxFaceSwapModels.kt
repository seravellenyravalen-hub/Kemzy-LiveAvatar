...
class ArcFaceEmbedder(
    ...
    override fun embedding(alignedSourceFace: Bitmap): FloatArray {
        ...
        val inputName = engine.inputNames().firstOrNull()
            ?: error("ArcFace model exposes no input.")
        val tensor = OnnxImageTensor.tensor(
            env,
            OnnxImageTensor.rgbNchw(alignedSourceFace, 112),
            1L, 3L, 112L, 112L
        )
        tensor.use {
            val output = engine.run(mapOf(inputName to tensor)).values.firstOrNull()
                ?: error("ArcFace model returned no output.")
...
/**
 * INSwapper adapter. The model is expected to expose one [1,3,128,128] input,
 * one [1,512] source-latent input, and one image output.
 */
class InswapperOnnx(
    private val engine: InferenceEngine
) : FaceSwapper {
    override fun swap(alignedTargetFace: Bitmap, sourceEmbedding: FloatArray): Bitmap {
        ...
        } ?: engine.inputNames().firstOrNull()
            ?: error("INSwapper model exposes no inputs.")
        val latentInput = engine.inputNames().firstOrNull { it != imageInput }
            ?: error("INSwapper model must expose image and source-latent inputs.")

        val imageTensor = OnnxImageTensor.tensor(
            env,
            OnnxImageTensor.rgbNchw(alignedTargetFace, InswapperModelSpec.faceWidth),
            1L, 3L, 128L, 128L
        )
        ...
        imageTensor.use {
            latentTensor.use {
                val output = engine.run(
                    mapOf(imageInput to imageTensor, latentInput to latentTensor)
                ).values.firstOrNull() ?: error("INSwapper returned no output.")
